...
`)}
      ...
      export default extensions
    `;try{await E.outputFile(m.resolve(e,"admin","src","extensions","_main-entry.ts"),a)}catch(i){c.panic("Failed to write the extensions entry file. See the error below for details:",{error:i})}}s(De,"createMainExtensionsEntry");async function R({appDir:e,dest:t,plugins:o}){let r=await Ae(e,t),n=Pe(o);await De(t,n,r),await _e(t,n)}s(R,"createEntry");async function Fe(e){let t=G.resolve(__dirname,"..","ui"),o=G.resolve(e,"admin");try{await We.copy(t,o,{filter:P})}catch(r){c.panic(`Could not copy the admin UI to ${o}. See the error below for details:`,{error:r})}}s(Fe,"copyAdmin");async function k({appDir:e,plugins:t}){let o=G.resolve(e,".cache");return await Fe(o),await R({appDir:e,dest:o,plugins:t}),{cacheDir:o}}s(k,"createCacheDir");import ie from"dotenv";import Te from"fs-extra";import ae from"node:path";var Ie=/^MEDUSA_ADMIN_/i,$="";switch(process.env.NODE_ENV){case"production":$=".env.production";break;case"staging":$=".env.staging";break;case"test":$=".env.test";break;case"development":default:$=".env";break}Te.existsSync($)?ie.config({path:ae.resolve(process.cwd(),$)}):$!==".env"&&ie.config({path:ae.resolve(process.cwd(),".env")});var q=s(e=>{let t=Object.keys(process.env).filter(r=>Ie.test(r)).reduce((r,n)=>(r[n]=process.env[n],r),{ADMIN_PATH:e.path||"/",NODE_ENV:e.env||"development",MEDUSA_BACKEND_URL:e.backend||process.env.MEDUSA_BACKEND_URL});return{"process.env":Object.keys(t).reduce((r,n)=>(r[n]=JSON.stringify(t[n]),r),{})}},"getClientEnv");import Re from"chokidar";import Ve from"fs-extra";import C from"node:path";async function B(e,t,o){let r=C.resolve(e,"src","admin"),n=Re.watch(r,{ignored:/(^|[/\\])\../,ignoreInitial:!0});n.on("all",async(a,i)=>{(a==="unlinkDir"||a==="unlink")&&Le(i,e,t),await R({appDir:e,dest:t,plugins:o}),c.info("Extensions cache directory was re-initialized")}),process.on("SIGINT",async()=>{await n.close()}).on("SIGTERM",async()=>{await n.close()})}s(B,"watchLocalAdminFolder");function Le(e,t,o){let r=C.resolve(t,"src","admin"),n=C.relative(r,e),a=C.resolve(o,"admin","src","extensions"),i=C.resolve(a,n);try{Ve.removeSync(i)}catch(d){c.error(`An error occurred while removing ${i}: ${d}`)}}s(Le,"removeUnlinkedFile");import Be from"fs-extra";import Je from"node:path";import Ke from"webpack";function se(e){let{options:t}=e;t.path&&(t.path.startsWith("/")||c.panic("'path' in the options of `@medusajs/admin` must start with a '/'"),t.path!=="/"&&t.path.endsWith("/")&&c.panic("'path' in the options of `@medusajs/admin` cannot end with a '/'"),typeof t.path!="string"&&c.panic("'path' in the options of `@medusajs/admin` must be a string"))}s(se,"validateArgs");import Ne from"@pmmmwh/react-refresh-webpack-plugin";import Oe from"copy-webpack-plugin";import Ue from"html-webpack-plugin";import K from"mini-css-extract-plugin";import V from"node:path";import{SwcMinifyWebpackPlugin as ze}from"swc-minify-webpack-plugin";import Me from"webpack";import Ge from"webpackbar";var J=["react","react-dom","react-router-dom","react-dnd","react-dnd-html5-backend","react-select","react-helmet-async","@tanstack/react-query","@tanstack/react-table","@emotion/react","medusa-react","@medusajs/ui","@medusajs/icons","@medusajs/ui-preset"];var ce=J.reduce((e,t)=>(e[`${t}$`]=S.resolve(t),e),{});function qe(e){return e?e==="/"||e.endsWith("/")?e:`${e}/`:"/app/"}s(qe,"formatPublicPath");function X({entry:e,dest:t,cacheDir:o,env:r,options:n,template:a,publicFolder:i,reporting:d="fancy"}){let f=r==="production",u=q({env:r,backend:n==null?void 0:n.backend,path:n==null?void 0:n.path}),l=qe(n==null?void 0:n.path),y=f?[new K({filename:"[name].[chunkhash].css",chunkFilename:"[name].[chunkhash].css"}),new Ge({basic:d==="minimal",fancy:d==="fancy"})]:[new K,new Ne];return{mode:r,devtool:f?!1:"inline-source-map",entry:[e],output:{path:t,filename:f?"[name].[contenthash:8].js":"[name].bundle.js",chunkFilename:f?"[name].[contenthash:8].chunk.js":"[name].chunk.js"},optimization:{minimize:!0,minimizer:[new ze],moduleIds:"deterministic",runtimeChunk:!0},module:{rules:[{test:/\.tsx?$/,exclude:/node_modules/,include:[o],use:{loader:"swc-loader",options:{jsc:{parser:{syntax:"typescript",jsx:!0},transform:{react:{runtime:"automatic",development:!f,refresh:!f}}}}}},{test:/\.jsx?$/,exclude:/node_modules/,include:[o],use:{loader:"swc-loader",options:{jsc:{parser:{syntax:"ecmascript",jsx:!0},transform:{react:{runtime:"automatic"}}}}}},{test:/\.css$/,use:[K.loader,"css-loader","postcss-loader"]},{test:/\.svg$/,oneOf:[{type:"asset/resource",resourceQuery:/url/},{type:"asset/inline",resourceQuery:/base64/},{issuer:/\.[jt]sx?$/,use:["@svgr/webpack"]}],generator:{filename:`images/${f?"[name]-[hash][ext]":"[name][ext]"}`}},{test:/\.(eot|otf|ttf|woff|woff2)$/,type:"asset/resource"},{test:/\.m?jsx?$/,resolve:{fullySpecified:!1}}]},resolve:{alias:ce,symlinks:!1,extensions:[".js",".jsx",".ts",".tsx"],mainFields:["browser","module","main"],modules:["node_modules",V.resolve(__dirname,"..","node_modules")],fallback:{readline:!1,path:!1}},plugins:[new Ue({inject:!0,template:a||V.resolve(__dirname,"..","ui","index.html"),publicPath:l}),new Me.DefinePlugin(u),new Oe({patterns:[{from:i||V.resolve(__dirname,"..","ui","public"),to:V.resolve(t,"public")}]}),...y].filter(Boolean),stats:f?"errors-only":"errors-warnings"}}s(X,"getWebpackConfig");async function A(e,t){se(t);let o=X(t),r=Je.join(e,"src","admin","webpack.config.js");if(await Be.pathExists(r)){let a;try{a=S(r)}catch(i){c.panic("An error occured while trying to load your custom Webpack config. See the error below for details:",{error:i})}typeof a=="function"&&(t.devServer&&(o.devServer=t.devServer),o=a(o,Ke),o||c.panic("Nothing was returned from your custom webpack configuration"))}return o}s(A,"getCustomWebpackConfig");function le(e){return(t,o)=>e(t,o)}s(le,"withCustomWebpackConfig");async function de({appDir:e,buildDir:t,plugins:o,options:r,reporting:n="fancy"}){await k({appDir:e,plugins:o});let a=Z.resolve(e,".cache"),i=Z.resolve(a,"admin","src","main.tsx"),d=Z.resolve(e,t),u=await A(e,{entry:i,dest:d,cacheDir:a,env:"production",options:r,reporting:n}),l=Xe(u);return new Promise((y,x)=>{l.run((p,h)=>{p&&(p.details&&c.error(p.details),x(p));let w=h.toJson();return h.hasErrors()&&c.error(JSON.stringify(w.errors)),y({stats:h,warnings:w.warnings})})})}s(de,"build");import fe from"fs-extra";import ue from"node:path";async function pe({appDir:e,outDir:t}){let o=ue.resolve(e,".cache","admin"),r=ue.resolve(e,t);await fe.remove(r),await fe.remove(o)}s(pe,"clean");import me from"node:path";import Ze from"react-dev-utils/openBrowser";import He from"webpack";import Qe from"webpack-dev-server";async function he({appDir:e,buildDir:t,plugins:o,options:r={path:"/",backend:"http://localhost:9000",develop:{open:!0,port:7001,logLevel:"error",stats:"normal"}}}){let{cacheDir:n}=await k({appDir:e,plugins:o}),a=me.resolve(n,"admin","src","main.tsx"),i=me.resolve(e,t),f=await A(e,{entry:a,dest:i,cacheDir:n,env:"development",options:r}),u=He({...f,infrastructureLogging:{level:r.develop.logLevel},stats:r.develop.stats==="normal"?"errors-only":void 0}),l={port:r.develop.port,client:{logging:"none",overlay:{errors:!0,warnings:!1}},open:!1,onListening:r.develop.open?function(p){p||c.warn("Failed to open browser."),Ze(`http://localhost:${r.develop.port}${r.path?r.path:""}`)}:void 0,devMiddleware:{publicPath:r.path,stats:r.develop.stats==="normal"?!1:void 0},historyApiFallback:{index:r.path,disableDotRule:!0},hot:!0},y=new Qe(l,u);await s(async()=>{c.info(`Started development server on http://localhost:${r.develop.port}${r.path?r.path:""}`),await y.start()},"runServer")(),await B(e,n,o)}s(he,"develop");export{J as ALIASED_PACKAGES,de as build,pe as clean,he as develop,F as findAllValidRoutes,D as findAllValidSettings,W as findAllValidWidgets,L as forbiddenRoutes,N as injectionZones,c as logger,v as normalizePath,le as withCustomWebpackConfig};
